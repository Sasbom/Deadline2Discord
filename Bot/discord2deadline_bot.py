...
def get_job_cmd(jobid):
    job_proc = subprocess.run(
        [DEADLINE_CMD, "-JobSubmissionInfoFromJob", str(jobid)],
        capture_output=True,
        text=True,
    )
    job_out = job_proc.stdout
    plug_proc = subprocess.run(
        [DEADLINE_CMD, "-PluginSubmissionInfoFromJob", jobid],
        capture_output=True,
        text=True,
    )
...
def compose_resultembed(
    ...
    if "ping" in data_dict.keys():
        if names := data_dict["ping"]:
            ...
            if user_ids:
                users = ", ".join(f"<@{user_id.discordid}>" for user_id in user_ids)
...
def get_user_pingable(username: str):
    id = pg.get_user(DB, username=username)
    if id:
        id = id.discordid
    return bool(id)
...
async def server_task():
    ...
    while not client.is_closed():
        if MESSAGES.has_messages:
            await asyncio.wait(
                [message.create_async_task(channel) for message in MESSAGES.messages]
            )
...
async def server_task_cleanup_logs():
    ...
    while not client.is_closed():
        if GC_AUTO_ENABLED:
            channel.send(
                f"Checking for deleted jobs and deleting them. Any queries made during this time will not be responsive. :clock:\n"
...
async def register_command(interaction: discord.Interaction):
    ...
    if user:
        await interaction.response.send_message(
            "Your username has already been registered! :star:", ephemeral=True
        )
    else:
        buser = pg.bot_user(name, interaction.user.id, ["user"])
        pg.insert_user(DB, buser)
        await interaction.response.send_message(
            "Your username has succesfully been registered! :white_check_mark:",
            ephemeral=True,
        )
...
async def deregister_command(interaction: discord.Interaction):
    ...
    user = pg.get_user(DB, username=name)
    if user:
        pg.remove_user(DB, user)
        await interaction.response.send_message(
            "Your username has been deregistered! :fire:", ephemeral=True
        )
    else:
        await interaction.response.send_message(
            "Your username doesn't exist in the registry. :nail_care:", ephemeral=True
        )
...
async def stats_to_embed_async(job_id):
    ...
    job_object = pg.get_job(DB, deadline_id=job_id)
    running_time_raw = get_timedelta(job_object.started)
    ...
    if render_time == "Not done yet.":
        ...
        time_pertask = float(parse_deadlinetime_as_seconds(running_time_raw)) / max(
            float(tasks_complete), 0.001
        )
    ...
    embed_msg = discord.Embed(
        title=f":chart_with_upwards_trend: Stats for `{name}`",
    ...
    embed_msg.add_field(
        name=":pencil: Task info:",
        value=f"**Processed** {tasks_complete} **out of** {tasks_total} **tasks.**"
...
job_group = app_commands.Group(
    name="job", description="All commands that have something to do or manipulate jobs"
)
...
async def renderjob_stats(interaction: discord.Interaction, job_name: str):
    ...
    if job_info:
        ...
        if name in owners or owners == "everyone":
            ...
            response_stat = CON.Jobs.CalculateJobStatistics(job_info.deadline_id)
            response_stat = str(response_stat).replace("'", '"')
            response_stat = response_stat.replace("None", '"None"')

            # msg = stats_to_embed(response_stat)
            msg = await stats_to_embed_async(job_info.deadline_id)
...
@job_group.command(name="requeue", description="Requeue all tasks in an existing job")
async def renderjob_requeue(interaction: discord.Interaction, job_name: str):
    ...
    if job_info:
        ...
        if name in owners or owners == "everyone":
            await interaction.response.defer(ephemeral=True, thinking=True)
            CON.Jobs.UpdateJobSubmissionDate(job_info.deadline_id)
            CON.Jobs.RequeueJob(job_info.deadline_id)
            await interaction.followup.send(
                f"Requested to requeue all tasks in `{job_name}`.", ephemeral=True
            )
        else:
            await interaction.response.send_message(
                "Your username is not associated with this job.", ephemeral=True
...
@job_group.command(
    name="fail", description="Deliberately Fail all tasks in an existing job"
)
async def renderjob_fail(interaction: discord.Interaction, job_name: str):
    ...
    if job_info:
        ...
        if name in owners or owners == "everyone":
            await interaction.response.defer(ephemeral=True, thinking=True)
            CON.Jobs.FailJob(job_info.deadline_id)
            await interaction.followup.send(
                f"Requested to fail all tasks in `{job_name}`.", ephemeral=True
            )
        else:
            await interaction.response.send_message(
                "Your username is not associated with this job.", ephemeral=True
...
@job_group.command(name="suspend", description="Suspend a job, putting it on pause.")
async def renderjob_suspend(interaction: discord.Interaction, job_name: str):
    ...
    if job_info:
        ...
        if name in owners or owners == "everyone":
            await interaction.response.defer(ephemeral=True, thinking=True)
            CON.Jobs.SuspendJob(job_info.deadline_id)
...
@job_group.command(
    name="resume", description="Resume a job, releasing it from suspended state"
)
async def renderjob_resume(interaction: discord.Interaction, job_name: str):
    name = interaction.user.name
    ...
    if job_info:
        ...
        if name in owners or owners == "everyone":
            await interaction.response.defer(ephemeral=True, thinking=True)
            CON.Jobs.ResumeJob(job_info.deadline_id)
...
async def renderjob_reschedule(
    ...
    if job_info:
        if job_name == job_new_name:
            await interaction.response.send_message(
                f"Please rename your job something else than `{job_name}` for clarity.",
                ephemeral=True,
            )
            return
        ...
        if name in owners or owners == "everyone":
            ...
            props, plug = get_job_cmd(job_info.deadline_id)
            ...
            if job_new_priority:
                if job_new_priority >= 0 and job_new_priority <= 100:
                    ...
                    await interaction.followup.send(
                        f"New given priority {job_new_priority} is invalid.\nPlease enter a whole number between 0 and 100."
                    )
                    return
            ...
            if job_new_frames:
                ...
                if not all(re.fullmatch(REGEX_FRAMERANGE, m.strip()) for m in split_f):
                    await interaction.followup.send(
                        f"Given framerange `{job_new_frames}` is not valid.\nUse valid syntax; `15, 20-25, 100`.",
                        ephemeral=True,
                    )
                    return
                else:
            ...
            plug["OutputFile"] = os.path.join(
                props["OutputDirectory0"], props["OutputFilename0"]
            )
            ...
            await interaction.followup.send(
                f"Requested to reschedule tasks in `{job_name}` as `{job_new_name}`.",
                ephemeral=True,
            )
            return
        else:
...
@job_group.command(
    name="mine", description="Show owned jobs and their states. Can take a while!"
)
async def renderjob_showmine(interaction: discord.Interaction):
    ...
    if job_info:
        ...
        job_responses, _ = await asyncio.wait(
            [
                asyncio.create_task(
                    generate_jobinfo_string(
                        job.deadline_id, job.deadline_name, loop=event_loop
                    )
                )
                for job in job_info
            ]
...
activehours_group = app_commands.Group(
    name="activehours", description="Options for active hours of job", parent=job_group
)
...
async def job_set_activehours(
    interaction: discord.Interaction,
    job_name: str,
    time_suspend: str = "08:00",
    time_resume: str = "19:00",
):
    name = interaction.user.name

    if not re.fullmatch(REGEX_TIME_HHMM, time_suspend):
        await interaction.response.send_message(
            f"Suspend time improperly formatted. `{time_suspend}` doesn't conform to HH:MM, 6:00, 00:00, 23:59, etc.",
            ephemeral=True,
        )
        return
    if not re.fullmatch(REGEX_TIME_HHMM, time_resume):
        await interaction.response.send_message(
            f"Resume time improperly formatted. `{time_resume}` doesn't conform to HH:MM, 6:00, 00:00, 23:59, etc.",
            ephemeral=True,
        )
        return
    ...
    if job_info:
        ...
        if name in owners or owners == "everyone":
            ...
            job_info.officehours_start = time_suspend
            job_info.officehours_end = time_resume
            job_info.officehours = flag
            pg.update_job(DB, job_info)

            await interaction.followup.send(
                f"Set `{job_name}` to activate at `{time_resume}` and pause at `{time_suspend}`.",
                ephemeral=True,
            )
        else:
            await interaction.response.send_message(
...
async def job_rem_activehours(interaction: discord.Interaction, job_name: str):
    name = interaction.user.name
    ...
    if job_info:
        ...
        if name in owners or owners == "everyone":
            await interaction.response.defer(ephemeral=True, thinking=True)
            try:
                job_info.officehours = False
                pg.update_job(DB, job_info)
            except BaseException as e:
                print(e)
            await interaction.followup.send(
                f"Set `{job_name}` to no longer be susceptible to suspension/resuming scheduling.",
                ephemeral=True,
            )
        else:
            await interaction.response.send_message(
...
@job_group.command(
    name="finish", description="Deregister a Completed/Failed/Deleted job that you own."
)
    ...
    if job_info:
        ...
        if name in owners or owners == "everyone":
            ...
            if status in ["Completed", "Failed", None]:
                ...
                pg.update_job(DB, job_info)
                await interaction.followup.send(
                    f"Unregistered `{job_name}` from internal registry!", ephemeral=True
                )
            else:
                await interaction.followup.send(
                    f"`{job_name}` couldn't be removed, status is **{status}**. \nTip: Use `/job fail {job_name}` to make it available for finishing!",
                    ephemeral=True,
                )
        else:
            await interaction.response.send_message(
...
async def job_deregister_all(interaction: discord.Interaction):
    ...
    if job_info:
        ...
        event_loop = asyncio.get_event_loop()
        # Generate a list of tasks to run
        deregister_responses, _ = await asyncio.wait(
            [
                asyncio.create_task(
                    deregister_job_async(
                        job.deadline_id, job.deadline_name, loop=event_loop
                    )
                )
                for job in job_info
...
prism_group = app_commands.Group(
    name="prism",
    description="Offers integration with Prism projects submitted to Deadline.",
)

# TODO PRISM INTEGRATION


@prism_group.command(
    name="register", description="Register the name of a Prism project."
)
async def create_prismproject(interaction: discord.Interaction, prism_project: str):
    ...
    if p is not None:
        await interaction.response.send_message(
            f"Prism project `{prism_project}` is already present in the system.",
            ephemeral=True,
        )
    else:
        DB.insert(
            {
                "prism_name": prism_project,
                "subscribed_users": "",
                "is_locked": "False",
                "prism_owner": user,
            }
...
@prism_group.command(
    name="deregister", description="Deregister the name of a Prism project."
)
async def remove_prismproject(interaction: discord.Interaction, prism_project: str):
    ...
    if p is not None and (p["prism_owner"] == user or is_admin):
        ...
        admin_msg = (
            "`Admin override`: " if (p["prism_owner"] != user and is_admin) else ""
        )
        await interaction.response.send_message(
            f"{admin_msg}Prism project `{prism_project}` has been removed from the system.",
            ephemeral=True,
        )
    elif p is not None and (p["prism_owner"] != user or not is_admin):
        await interaction.response.send_message(
            f"Prism project `{prism_project}` is not yours! You can't remove it.",
            ephemeral=True,
        )
    else:
        await interaction.response.send_message(
            f"No records of Prism project `{prism_project}` found.", ephemeral=True
        )


@prism_group.command(name="subscribe", description="Subscribe to a Prism project")
async def user_join_prismproject(interaction: discord.Interaction, prism_project: str):
    ...
    if p is not None and p["is_locked"] == "False":
        users: list[str] = (
            p["subscribed_users"].split(",") if p["subscribed_users"] else []
        )
        if user in users:
            await interaction.response.send_message(
                f"You are already subscribed to `{prism_project}`.", ephemeral=True
            )
        else:
            users.append(user)
            DB.update(
                {"subscribed_users": ",".join(users)},
                project.prism_name == prism_project,
            )
            await interaction.response.send_message(
                f"Succesfully subscribed to `{prism_project}`!", ephemeral=True
            )
    elif p is not None and p["is_locked"] == "True":
        await interaction.response.send_message(
            f"Prism Project: `{prism_project}` is not able to be subscribed to, because the owner locked it.",
            ephemeral=True,
        )
    else:
        await interaction.response.send_message(
            f"Register Prism project `{prism_project}` before subscribing to it!",
            ephemeral=True,
        )


@prism_group.command(name="unsubscribe", description="Unsubscribe from a Prism project")
async def user_leave_prismproject(interaction: discord.Interaction, prism_project: str):
    ...
    if p is not None and p["is_locked"] == "False":
        ...
        if user not in users:
            await interaction.response.send_message(
                f"You are not subscribed to `{prism_project}`.", ephemeral=True
            )
        elif user in users:
            users = [
                u for u in users if not u == user
            ]  # Recompose list without user in it
            DB.update(
                {"subscribed_users": ",".join(users)},
                project.prism_name == prism_project,
            )
            await interaction.response.send_message(
                f"Succesfully unsubscribed from `{prism_project}`!", ephemeral=True
            )
    elif p is not None and p["is_locked"] == "True":
        await interaction.response.send_message(
            f"Prism Project: `{prism_project}` is not able to be unsubscribed from, because the owner locked it.",
            ephemeral=True,
        )
    else:
        await interaction.response.send_message(
...
@prism_group.command(
    name="lock", description="Lock Prism project, allowing no more subscribers."
)
async def lock_prismproject(interaction: discord.Interaction, prism_project: str):
    ...
    if p is not None and (p["prism_owner"] == user or is_admin):
        ...
        await interaction.response.send_message(
            f"{admin_msg}Prism project `{prism_project}` has been locked!\nNo one can subscribe/unsubscribe anymore.",
            ephemeral=True,
        )
    elif p is not None and (p["prism_owner"] != user or not is_admin):
        await interaction.response.send_message(
            f"Prism project `{prism_project}` is not yours! You can't lock it.",
            ephemeral=True,
        )
    else:
        await interaction.response.send_message(
...
@prism_group.command(
    name="unlock", description="UnLock Prism project, allowing subscribers."
)
async def lock_prismproject(interaction: discord.Interaction, prism_project: str):
    prism_project = prism_project.strip()  # normalize name
    user = interaction.user.name
    ...
    if p is not None and (p["prism_owner"] == user or is_admin):
        ...
        await interaction.response.send_message(
            f"{admin_msg}Prism project `{prism_project}` has been unlocked!\nPeople can subscribe/unsubscribe again.",
            ephemeral=True,
        )
    elif p is not None and (p["prism_owner"] != user or not is_admin):
        await interaction.response.send_message(
            f"Prism project `{prism_project}` is not yours! You can't unlock it.",
            ephemeral=True,
        )
    else:
        await interaction.response.send_message(
    ...
    if prism_info:
        ...
        for p in prism_info:
            locked = (
                "`Locked` :locked:"
                if p["is_locked"] == "True"
                else "`Unlocked` :unlock:"
            )
            owner = p["prism_owner"]
            subs = (
                ", ".join(p["subscribed_users"].split(","))
                if p["subscribed_users"]
                else "No one has subbed yet..."
            )
            response_txt.append(
                f"> `{p['prism_name']}`,  Owned by: `{owner}`, {locked}\n> - Subscribers: `{subs}`\n"
            )
        await interaction.followup.send("\n".join(response_txt))
    else:
        await interaction.response.send_message(
            "No Prism projects were found. :skull:", ephemeral=True
        )
...
async def calculate_frames_fromseconds(
    interaction: discord.Interaction,
    fps: str = "24.0",
    ...
    if re.fullmatch(REGEX_FLOAT, fps):
        ...
        await interaction.response.send_message(
            "Please enter a valid number for fps, frames per second. This can be a decimal number e.g. (23.994)",
            ephemeral=True,
        )
        return
    ...
    for i, t in enumerate(timecodes):
        if not re.fullmatch(REGEX_TIME_HHMMSS, t):
            if re.fullmatch(REGEX_TIME_MMSS, t):
                ...
                await interaction.response.send_message(
                    f"Time: {t} could not be properly interpreted.\nPlease format time as HH:MM:SS, or MM:SS e.g. (01:05:54, 12:10, 1:30).",
                    ephemeral=True,
                )
                return
...
farm_group = app_commands.Group(
    name="farm", description="Manage the farm in different ways."
)
garbage_collect_group = app_commands.Group(
    name="garbagecollect", description="Garbage collection settings", parent=farm_group
)
...
async def auto_gc(
    interaction: discord.Interaction, enabled: bool, hour_interval: Optional[int] = None
):
...
async def force_gc(interaction: discord.Interaction):
    ...
    channel: discord.TextChannel = client.get_channel(SECRET.channel)
    await channel.send(
        "Checking for deleted jobs and deleting them. Any queries made during this time will not be responsive. :clock:"
    )
    await garbage_collect_async()
